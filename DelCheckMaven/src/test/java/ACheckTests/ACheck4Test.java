...
public class ACheck4Test {
    ...
    int[] tokenTypes = new int[]{TokenTypes.LCURLY, TokenTypes.LPAREN, 
	 ...
	 @Test
	void beginTreeTest() {
			ACheck4 spyDifficulty = spy(new ACheck4());
	...
	void getDefaultTokensTest() {
		ACheck4 difficulty = new ACheck4();
   ...
   void visitTokenBORTest() {
		...
		DetailAST mockAST = mock(DetailAST.class);
	  	...
	  	spyDifficulty.visitToken(mockAST);
	  	verify(spyDifficulty, atLeastOnce()).visitToken(mockAST);
   }
   ...
   void finishTreeTest() {
		ACheck4 spyDifficulty = spy(new ACheck4());
		DetailAST mockAST = mock(DetailAST.class);
		assertEquals(0, spyDifficulty.uniqRand.size());
		
		
   	String message = "Halstead Difficulty: 0 -LS";
   ...
   void finishTreeNotEqualTest() {
	   ...
	   spyDifficulty.uniqRand.add(mockAST.getText());
	   doNothing().when(spyDifficulty).log(0, message);
