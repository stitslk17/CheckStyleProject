...
public class ACheck5 extends AbstractCheck {
//mVariable, mAge, mStudentID
    ...
    int length = 0;
    int halVol = 0;
    public Set<String> uniqOr = new HashSet<String>();
    int totalRand = 0;
    public Set<String> uniqRand = new HashSet<String>();
    int halDif = 0;
    ...
    public void finishTree(DetailAST aAST) {
    	halVol = (int)((int)length*(Math.log(uniq.size()) / Math.log(2)));
    	if (uniqRand.size() > 0)
    	{
        	halDif = (uniqOr.size()/2)*(totalRand)/(uniqRand.size());
    	}
    	count = halVol * halDif;
